// manager.js - Panel Administrador
...
window.calcTrip = async () => {
  ...
  try {
    ...
    const resDiv = document.getElementById('manager-trip-result');
    if (resDiv) {
      resDiv.style.display = 'block';
      document.getElementById('manager-trip-cost-display').textContent = resultado.costo;
      document.getElementById('manager-trip-distance-display').textContent = resultado.distance;
      document.getElementById('manager-trip-duration-display').textContent = resultado.duration;
    }
    window._managerTrip = { ...resultado, origen, destino, personas };
...
window.createTrip = async () => {
  if (!window._managerTrip) return showAlert('Primero calculá el costo', 'warning');
  const select = document.getElementById('trip-chofer');
  ...
  const datos = window._managerTrip;
  try {
    await addDoc(collection(db, 'trips'), {
      ...
      creadoPor: 'manager', fechaInicio: serverTimestamp()
    });
...
window.deleteAlert = async (id) => {
  try { await deleteDoc(doc(db, 'alerts', id)); loadAlerts(); showAlert('Alerta eliminada', 'success'); }
...
window.deleteTrip = async (id) => {
  ...
  try { await deleteDoc(doc(db, 'trips', id)); loadHistory(); showAlert('Viaje eliminado', 'success'); }
  catch (err) { showAlert(`Error: ${err.message}`, 'danger'); }
};
...
async function loadStats() {
  try {
    const [users, trips] = await Promise.all([getDocs(collection(db, 'users')), getDocs(collection(db, 'trips'))]);
